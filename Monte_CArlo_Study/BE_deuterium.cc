...
//This code wil read the rootfile from deuterium_second.cc and then will plot the histogram for Lambda_n
#include <iostream>
#include <iomanip>
#include <fstream>
#include <TRandom.h>
...
void BE_deuterium()
{
  ...
  t5->Add("./output_root/deuterium_second.root");// Al as target
 ...
 TH1F *h5 = new TH1F("h5"," D target and #Lambdan as Recoil ",500,-25.0,25.0);
 TH1F *h6 = new TH1F("h6"," D  as target and #Lambdan  as Recoil",100,-1.50e-3,1.5e-3);
 TH1F *h7 = new TH1F("h7"," D  as target and #Lambdan  as Recoil",100,-1.5e-3,1.5e-3);
 TH1F *h8 = new TH1F("h8"," D  as target and #Lambdan  as Recoil",100,-1.5e-3,1.5e-3);
 TH1F *h9 = new TH1F("h9"," D  as target and #Lambdan  as Recoil  ",100,-20.0e-3,20.0e-3);
 TH1F *h10 = new TH1F("h10"," D as target,and #Lambdan  as Recoil",100,-20.0e-3,20.0e-3);
 TH1F *h11 = new TH1F("h11"," D as target, and #Lambdan  as Recoil",100,-30.0e-3,30.0e-3);
 ...
 h5->GetYaxis()->CenterTitle();
  ...
  TF1 *f5 = new TF1("f5","gaus",-2.5,2.5);
  ...
  l5.DrawLatex(-20,350,Form("#color[2]{mean = %.6g}",f5->GetParameter(1)));  
  l5.DrawLatex(-20,325,Form("#color[2]{#sigma = %.6g}",f5->GetParameter(2)));       
  l5.DrawLatex(-20,300,Form("#Lambdan"));
  l5.DrawLatex(-20,275,Form("#sigma_{E} = 6.7*10^{-5}  GeV"));
  l5.DrawLatex(-20,250,Form("#sigma_{Pee'} = #sigma_{Pk} = 1.0*10^{-4}  GeV"));
  l5.DrawLatex(-20,225,Form("#sigma_{#theta_{ee'}} = #sigma_{#theta_{ek}} = 3.4*10^{-3} Radian"));
  l5.DrawLatex(-20,200,Form("#sigma_{#Delta#phi} = 4.8*10^{-3} Radian")); 
