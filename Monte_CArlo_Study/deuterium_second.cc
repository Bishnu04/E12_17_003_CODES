//03/20/2020
//will read root files from DEUTERIUM-first.cc
// will generate the errors and will do the kinematics for the missing Mass(BE) for the deuterium target
...
void deuterium_second()
{
  TChain *t1 = new TChain("ktree");
  t1->Add("./output_root/DEUTERIUM_first.root");
  ...
  mt = 1.8756282; //Mass of Carbon target
  mk = 0.4937; 
  ...
  TFile *f2 = new TFile("./output_root/deuterium_second.root","recreate");
  f2->cd();
